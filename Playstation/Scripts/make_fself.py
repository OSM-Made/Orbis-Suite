...
# Python 3 compatibility for reading/writing binary data
if sys.version_info[0] < 3:
    raise RuntimeError("This script requires Python 3.")

def int_with_base_type(val):
    """Converts a string or integer value to an integer, handling base prefixes."""
    return int(val, 0)

def try_parse_int(x, base=0):
    """Attempts to parse a value as an integer, returns None on failure."""
    try:
...
def align_up(x, alignment):
    """Aligns x up to the nearest multiple of alignment."""
    return (x + (alignment - 1)) & ~(alignment - 1)

def align_down(x, alignment):
    """Aligns x down to the nearest multiple of alignment."""
...
def ilog2(x):
    ...
    # Using bit_length() is more idiomatic in Python 3 for this purpose
...
def check_file_magic(f, expected_magic):
    """Checks the magic number at the current file offset."""
    old_offset = f.tell()
    try:
...
def parse_version(version):
    """Parses version integer into a string format."""
...
def sha256(data):
    """Calculates SHA256 digest of byte data."""
    return hashlib.sha256(data).digest()

def hmac_sha256(key, data):
    """Calculates HMAC-SHA256 digest of byte data using a key."""
    return hmac.new(key=key, msg=data, digestmod=hashlib.sha256).digest()

class ElfError(Exception):
    """Custom exception for ELF file parsing errors."""
...
class ElfEHdr(object):
    """Represents the ELF Header (64-bit)."""
    FMT = '<4s5B6xB'
    EX_FMT = '<2HI3QI6H'

    MAGIC = b'\x7FELF' # Changed to bytes literal
    CLASS64 = 0x2
    DATA2LSB = 0x1
    EM_X86_64 = 0x3E
    EV_CURRENT = 0x1
    ...
    def load(self, f):
        """Loads ELF Header from a file handle."""
    ...
    def save(self, f):
        """Saves ELF Header to a file handle."""
    ...
    def load(self, f):
        ...
        if self.ehdr.has_sections():
            ...
            for i in range(self.ehdr.shnum):
                ...
                # NOTE: The original code used phdr.filesz here, which seems like a bug.
                # Assuming it should have been shdr.size for sections. We'll use shdr.size.
                if shdr.size > 0: 
...
# Changed to byte string literals
EMPTY_DIGEST = b'\0' * DIGEST_SIZE
EMPTY_SIGNATURE = b'\0' * SIGNATURE_SIZE

class SignedElfEntry(object):
    """Represents an entry (segment/digest) in the SELF file metadata."""
    FMT = '<4Q'

    # ... (Properties logic remains the same) ...
    ...
    def __init__(self, index):
        ...
        self.data = None # This will store the segment data (bytes)
    ...
    # Original had @wbits.setter here, which is wrong. Changed to @segment_index.setter
...
class SignedElfExInfo(object):
    """Represents extended information header."""
    FMT = '<4Q32s'
...
class SignedElfNpdrmControlBlock(object):
    """Represents NPDRM control block."""
    FMT = '<H14x19s13s'
...
class SignedElfFile(object):
    """Represents the entire signed SELF file."""
    COMMON_HEADER_FMT = '<4s4B'
    EXT_HEADER_FMT = '<I2HQ2H4x'

    MAGIC = b'\x4F\x15\x3D\x1D' # Changed to bytes literal
    VERSION = 0x00
    MODE = 0x01
    ENDIAN = 0x01
    ATTRIBS = 0x12
    ...
    def __init__(self, elf, **kwargs):
        ...
        # Arguments are passed as keyword arguments, defaults are applied if missing or None
    ...
    def _prepare(self):
        """Calculates sizes, offsets, and initializes metadata structures."""
        ...
        if SignedElfFile.HAS_NPDRM:
            ...
            # Changed to use byte literals (b'\0')
        ...
        if not self.auth_info is None:
            # Struct pack returns bytes. ljust() must pad with bytes (b'\0').
        ...
        for i in range(self.elf.ehdr.phnum):
            ...
            # Use print() function
            print(f'processing segment #{i:02}...')
    ...
    def save(self, f):
        """Writes the SELF file structure and content to a file handle."""
        start_offset = f.tell()
        ...
        # Use BytesIO to simulate writing the ELF header data without affecting the main file position
        # This is safer than seeking back and forth in the main file writer
        temp_buffer = BytesIO()
        self.elf.save(temp_buffer, True)
        temp_buffer.seek(0)
        
        # Write the aligned ELF header data
        f.write(temp_buffer.read().ljust(elf_header_size, b'\0'))
...
def ensure_hex_string(val, **kwargs):
    """
    Ensures the input is a valid hexadecimal string, converts it to bytes,
    ...
    val_size_in_hex_chars = len(val)

    # Check if the string is valid hexadecimal
    # Using str.isalnum is an approximation; we check against hexdigits explicitly
    if val_size_in_hex_chars % 2 != 0 or not all(c.lower() in string.hexdigits for c in val):
        return None
        
    if val_size_in_hex_chars > 0:
        # Python 3: use bytes.fromhex() instead of str.decode('hex')
...
# The parsing logic remains mostly the same, just changing print statements
# and using f-strings for error messages.
...
parser.add_argument('--auth-info', type=auth_info_type, default=None, help='authentication info (0x88 bytes hex string)')

# The main logic needs to be guarded by a check for arguments
if len(sys.argv) == 1:
    parser.print_usage()
    sys.exit(1)
...
if not args.ptype is None:
    ...
    if ptype is None:
        parser.error(f'invalid program type: {args.ptype}') # Changed error message for clarity
...
if not (0 <= app_version <= 0xFFFFFFFFFFFFFFFF):
    parser.error(f'invalid application version: 0x{app_version:016X}')

fw_version = args.fw_version
if not (0 <= fw_version <= 0xFFFFFFFFFFFFFFFF):
    parser.error(f'invalid firmware version: 0x{fw_version:016X}')
...
print(f'loading elf file: {elf_file_path}')
try:
    # Use 'rb' for binary reading
    ...
    traceback.print_exc()
    print('')
    parser.error(f'unable to load elf file: {elf_file_path} ({err})')

print(f'saving fake signed elf file: {fself_file_path}')
try:
    # Use 'wb' for binary writing
    ...
    traceback.print_exc()
    print('')
    parser.error(f'unable to save fself file: {fself_file_path} ({err})') # Corrected path in error message
