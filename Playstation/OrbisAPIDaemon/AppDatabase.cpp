...
sqlite3* AppDatabase::OpenDatabase()
{
	...
	if (res != SQLITE_OK)
	{
		Logger::Error("OpenDatabase(): Failed because %s", sqlite3_errmsg(db));
		return nullptr;
	}
	else
	{
...
int AppDatabase::GetVersion()
{
	...
	if (res != SQLITE_OK)
	{
		Logger::Error("sqlite3_prepare(): Failed because %s", sqlite3_errmsg(db));
		sqlite3_close(db);
		return false;
	}
	...
	if (res != SQLITE_DONE) {
		printf("GetVersion(): Res %d Error: %s", res, sqlite3_errmsg(db));
	}
...
bool AppDatabase::GetApps(std::vector<AppInfo>& Apps)
{
	...
	if (res != SQLITE_DONE) {
		printf("GetApps(): Res %d Error: %s", res, sqlite3_errmsg(db));
	}
...
bool AppDatabase::GetAppInfoString(const char* TitleId, char* Out, size_t OutSize, const char* Key)
{
	...
	if (res != SQLITE_DONE) {
		printf("GetAppInfoString(): Res %d Error: %s", res, sqlite3_errmsg(db));
	}
...
bool AppDatabase::SetVisibility(const char* TitleId, VisibilityType Visibility)
{
	...
	if (res != SQLITE_OK)
	{
		Logger::Error("sqlite3_exec(): Failed because %s", sqlite3_errmsg(db));
		sqlite3_close(db);
		return false;
	}
